...
支线 C：CI/CD 管理：本地 diff -> AI 单测补丁 -> 审批 -> pytest 回归 -> 质量结论
```
...
目标：让本地代码变更形成测试补全和回归证据。V1 的 CI/CD 管理是本地 diff 与回归证据工作流，不是 GitHub Actions/GitLab 云平台。
...
- CI/CD 管理能完成 diff 分析、patch 评审、pytest 回归、报告生成。
