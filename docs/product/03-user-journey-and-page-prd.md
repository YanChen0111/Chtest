...
用户输入：项目、模块、需求文本或 Markdown 文件、需求来源、目标测试类型、是否使用外部知识库 evidence。
...
2. 用户点击“开始需求评审”。
3. 系统创建 AITask。
4. RequirementReviewAgent 输出六维评分、风险矩阵、澄清问题、可测性建议。
5. 用户确认或编辑需求评审结果。
6. 用户点击“生成用例”。
7. CaseGenerationAgent 生成候选用例。
8. 候选用例进入 Case Generation Review 页面。
9. 用户接受、编辑后接受、驳回或要求优化。
10. 通过评审的候选用例进入 Test Case Library。
11. 系统记录批次质量指标。
...
| 区域 | 内容 |
|---|---|
| 左侧输入 | 需求文本、文件、模块、测试类型 |
| 中间评分 | 六维评分、总分、风险等级 |
| 右侧问题 | 澄清问题、冲突、不可测描述、改写建议 |
