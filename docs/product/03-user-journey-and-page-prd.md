...
主线 B：需求/用例 -> AI AutomationDraft -> 审批 -> pytest/Playwright 执行 -> 报告
支线 C：Git diff -> AI UnitTestPatch -> 审批 -> pytest 回归 -> 质量结论
```
...
入口页面：AI Workbench 或 Requirement Review。

用户输入：项目、模块、需求文本或 Markdown 文件、需求来源、目标测试类型、可选 ContextArtifact、是否使用外部知识库 evidence。
...
2. 用户可选择 ContextArtifact，例如 API notes、OpenAPI 片段、fixture、日志摘要。
3. 用户点击“开始需求评审”。
4. 系统创建 AITask，并记录 `context_artifact_ids`。
5. RequirementReviewAgent 输出六维评分、风险矩阵、澄清问题、可测性建议。
6. 用户查看本次实际使用的 ContextArtifact。
7. 用户确认或编辑需求评审结果。
8. 用户点击“生成用例”。
9. CaseGenerationAgent 生成候选用例。
10. 候选用例进入 Case Generation Review 页面。
11. 用户接受、编辑后接受、驳回或要求优化。
12. 通过评审的候选用例进入 Test Case Library。
13. 系统记录批次质量指标。
...
入口页面：Test Case Library、Automation Draft Center、AI Workbench。

第一版支持：

- 从已通过评审的 TestCase 生成 pytest AutomationDraft。
- 从已通过评审的 UI TestCase 生成 Playwright AutomationDraft。
- 审批或编辑 AutomationDraft。
...
3. 草稿进入 Automation Draft Center。
4. 用户查看代码、执行说明、风险说明。
...
入口页面：Git Quality Center。

第一版输入方式：本地仓库路径、base/head commit、手动上传 diff。GitHub webhook、GitHub MCP、PR 评论是 V2 能力。
...
2. GitTool 读取变更文件和 diff。
3. GitDiffAgent 输出变更摘要、风险等级、影响模块、建议测试范围。
...
12. ReportAgent 生成 GitQualityReport。
...
### 6.1 Home / AI Workbench

目标：统一发起 AI 测试任务，查看最近任务和关键指标。

| 区域 | 内容 |
|---|---|
| 顶部操作区 | 新建需求评审、新建用例生成、新建自动化草稿、新建 Git 质量分析 |
| 最近任务 | AITask 状态、Agent、耗时、结果入口 |
| 今日质量概览 | 用例采纳率、草稿通过率、执行通过率、回归通过率 |
| 待处理 | 待评审用例、待审批 AutomationDraft、待审批 patch、失败测试 |
...
| 区域 | 内容 |
|---|---|
| Project | 项目名称、描述、默认语言、默认测试类型 |
| Module Tree | 5 级模块树 |
| Repository | 本地仓库路径、默认 base branch |
| Environment | dev/test/local-prod 配置 |
| TestCommand | pytest、Playwright 命令 allowlist；Newman/JMeter 后置 |
| ContextArtifact | 本地上下文文档、API notes、OpenAPI 片段、fixture、日志摘要；展示 safe_to_show、redaction_applied |

验收：后续 AI 任务都能引用项目、模块、仓库、测试命令和已通过安全检查的 ContextArtifact。

### 6.3 Requirement Review

目标：让 AI 在测试开始前发现需求问题。

| 区域 | 内容 |
|---|---|
| 左侧输入 | 需求文本、文件、模块、测试类型、ContextArtifact 选择器、use_knowledge 开关 |
| 中间评分 | 六维评分、总分、风险等级 |
...
### 6.5 Test Case Library And Suites
...
### 6.7 Git Quality Center

目标：让本地代码变更形成测试补全和回归证据。
...
验收：每个 GitChangeSet 最终能给出“可合并/不建议合并/需人工判断”的质量结论。

### 6.8 Automation Execution Center
...
- 用户可以从 Home 进入三条最小闭环。
- Requirement Review 能生成可保存的六维评分和风险矩阵。
- Case Generation Review 能完成人工评审闭环。
- Automation Draft Center 能生成、评审、执行 pytest/Playwright 草稿。
- Git Quality Center 能完成 diff 分析、patch 评审、pytest 回归、报告生成。
- Report Center 能查看 Markdown/HTML/JSON 报告。
