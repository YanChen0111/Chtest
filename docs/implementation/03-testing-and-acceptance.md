...
```text
需求 -> AI 评审 -> 候选用例 -> 人工评审 -> AutomationDraft -> 审批 -> runner 执行 -> artifacts -> report
```
...
| Eval bench | pytest or CLI smoke | mock provider schema、evidence、unsafe output 指标 |
...
- Requirement 创建成功，并能绑定空列表或实际 context artifacts。
- Mock RequirementReviewAgent 生成六维评分和风险。
- Mock CaseGenerationAgent 生成候选用例。
...
- 用户能看到 AI 分析的需求和使用的 context artifacts。
- 用户能看到哪个候选用例被批准、编辑或拒绝。
...
- mock-provider eval bench 至少输出 schema_valid_rate、evidence_complete_rate、unsafe_output_rate。
