...
第一工程检查点是 V0.1 Walking Skeleton；第一产品验收对象是 V1 Minimum Demo。

V0.1 Walking Skeleton：

```text
Project -> ContextArtifact -> Mock AITask -> artifacts -> minimal pytest execution -> minimal report JSON
```
...
### 4.0 V0.1 Walking Skeleton

使用：`docs/implementation/00-v0.1-walking-skeleton.md`

通过标准：

- `/health` 和 `/ready` 通过。
- Project Settings bootstrap 可返回项目、模块、仓库、环境、测试命令。
- ContextArtifact 作为 Artifact 保存，并记录 `owner_entity_type=Project`。
- Mock AITask 记录 `context_artifact_ids`，并生成 `context_manifest.json`。
- 最小 pytest 执行或 TestRunner stub 生成 TestRun 和执行 artifact。
- 最小 `automation_execution` Report 引用 evidence manifest。
- V0.1 不替代完整 V1 Minimum Demo，不能绕过正式评审/审批规则。

### 4.1 V1 Minimum Demo Evidence Loop
