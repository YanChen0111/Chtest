...
| Fix frontend build TypeScript baseline | done | `npm --prefix frontend run build && npm --prefix frontend run test -- --run && git diff --check` | `07b1442` | narrow type fixes |
| Slice 23 completion gate | done | `npm --prefix frontend run build && npm --prefix frontend run test -- --run && git diff --check` | pending | docs and handoff |
