...
| Add contract smoke for generated-case evidence fields | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_contract_golden.py -q && git diff --check` | pending | schema/evidence proof only |
