...
| Add TestKnowledgeCard candidate review golden smoke | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_candidate_review_contract_golden.py -q && git diff --check` | `05913ea` | no CRUD/runtime |
| Slice 37 completion gate | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_candidate_review_contract_golden.py backend/app/tests/golden/test_test_knowledge_card_handoff_contract_golden.py -q && git diff --check` | pending | docs and handoff |
