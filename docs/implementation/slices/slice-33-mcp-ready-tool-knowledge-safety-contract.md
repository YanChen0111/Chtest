...
| Add MCP-ready tool/knowledge safety golden smoke | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_mcp_ready_tool_knowledge_safety_contract_golden.py -q && git diff --check` | pending | no runtime/provider behavior |
