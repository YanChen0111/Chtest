...
|---|---|---|---|---|
| Add knowledge feedback review gate task plan | done | `test -f docs/implementation/slices/slice-35-knowledge-feedback-review-gate-contract.md && rg -n "Knowledge Feedback Review Gate|KnowledgeFeedbackDraft|Product Value Answer|Non-goals|Task Table" docs/implementation/slices/slice-35-knowledge-feedback-review-gate-contract.md NEXT_AI_TASK.md && git diff --check` | pending | planning-only scope |
| Define KnowledgeFeedbackDraft review gate contracts | planned | `rg -n "KnowledgeFeedbackDraft|approve_feedback|reject_feedback|request_revision|mark_prompt_eligible|ReviewHistory|prompt eligibility|feedback review artifact" docs/contracts/01-data-model-contract.md docs/contracts/02-api-contract.md docs/contracts/03-state-machines.md docs/contracts/04-artifact-contract.md docs/implementation/slices/slice-35-knowledge-feedback-review-gate-contract.md && git diff --check` | pending | contract-only |
| Add knowledge feedback review gate golden smoke | planned | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_knowledge_feedback_review_gate_contract_golden.py -q && git diff --check` | pending | no review runtime |
| Slice 35 completion gate | planned | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_knowledge_feedback_review_gate_contract_golden.py backend/app/tests/golden/test_knowledge_feedback_contract_golden.py -q && git diff --check` | pending | docs and handoff |
