...
| Add TestKnowledgeCard prompt context consumption golden smoke | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_prompt_context_consumption_contract_golden.py -q && git diff --check` | pending | no prompt runtime |
