...
| Add Generated Case Human Review Decision golden smoke | done | `backend/.venv/Scripts/python.exe -m pytest backend/app/tests/golden/test_generated_case_human_review_decision_contract_golden.py -q && git diff --check` | pending | no approval/promotion/runtime integration |
