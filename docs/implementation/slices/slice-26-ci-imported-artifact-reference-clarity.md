...
| Add CI imported reference frontend clarity | done | `npm --prefix frontend run test -- --run src/views/cicd/CicdQualityCenterView.spec.ts && npm --prefix frontend run build && git diff --check` | pending | CI/CD page only |
