...
| Add KnowledgeAdapter provider evaluation plan golden smoke | done | `backend/.venv/Scripts/python.exe -m pytest backend/app/tests/golden/test_knowledge_adapter_provider_evaluation_plan_contract_golden.py -q && git diff --check` | `c234028` | no provider/runtime integration |
| Slice 50 completion gate | done | `backend/.venv/Scripts/python.exe -m pytest backend/app/tests/golden/test_knowledge_adapter_provider_evaluation_plan_contract_golden.py backend/app/tests/golden/test_mcp_ready_tool_knowledge_safety_contract_golden.py -q && git diff --check` | pending | docs and handoff |
