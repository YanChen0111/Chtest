...
| Add UnitTestPatch and QualityGateDecision model/schema | done | `backend/.venv/bin/python -m pytest backend/app/tests/api/test_unit_test_patch_regression.py -q` | `ae601ff` | persistence only |
...
| Add UnitTestPatch apply API | done | `backend/.venv/bin/python -m pytest backend/app/tests/api/test_unit_test_patch_regression.py -q` | pending commit | applies approved test-only patch artifact |
