...
|---|---|---|---|---|
| Add Local Review Attribution History task plan | planned | `test -f docs/implementation/slices/slice-21-local-review-attribution-history.md && rg -n "Local Review Attribution History|Product Value Answer|Non-goals|Task Table|append-only" docs/implementation/slices/slice-21-local-review-attribution-history.md` | pending | planning-only scope |
| Define review history contract boundary | planned | `rg -n "ReviewHistory|review history|review attribution|Default User|RBAC|permissions" docs/contracts docs/implementation/slices/slice-21-local-review-attribution-history.md` | pending | contract-only before code |
| Add review history model and service | planned | `backend/.venv/bin/python -m pytest backend/app/tests/api/test_review_history.py -q` | pending | append-only local events |
| Attach history to existing review actions | planned | `backend/.venv/bin/python -m pytest backend/app/tests/api/test_review_history.py backend/app/tests/api/test_automation_draft.py backend/app/tests/api/test_unit_test_patch_regression.py -q` | pending | no state-machine changes |
