# Slice 47: TestKnowledgeCard Prompt Context Discrepancy Resolution Review Contract Task Plan

## Goal

Define the prompt context discrepancy resolution review contract for future
human review of TestKnowledgeCard prompt context discrepancy records before any
...
broad TestKnowledgeCard CRUD, automatic prompt eligibility, or automatic
discrepancy remediation exists.

This slice is contract-first. It documents how a future resolution review may
acknowledge, reject, request clarification for, or mark discrepancy records as
resolved by later review while preserving discrepancy artifact ids, review
summary export artifact ids, affected citation ids, evidence gap summary,
mismatch reason, reviewer note, severity, resolution status, unresolved
follow-up flags, source hashes, context manifest references, PromptVersion,
SkillVersion, and ReviewHistory.

## Product Value Answer

After this slice, Chtest can describe how tracked prompt context discrepancies
are reviewed and resolved as auditable evidence without changing frontend
pages, report generation behavior, evidence storage, prompt assembly, provider
behavior, export endpoints, or retrieval runtime. Users get a clear contract
for resolution review decisions: resolution action, accepted/rejected/
acknowledged discrepancy ids, reviewer note, follow-up flags, failure behavior,
and ReviewHistory remain visible before any UI, report generator, provider, or
prompt runtime treats a discrepancy as closed.
...
- Current contracts do not define a resolution review boundary for accepting,
  rejecting, acknowledging, clarifying, or closing prompt context discrepancy
  records as later-review evidence.
...
- No artifact upload, artifact mutation, artifact delete, source artifact
  mutation, discrepancy artifact mutation outside declared resolution review
  evidence, review summary export mutation, audit review decision mutation,
  ...
  outside declared resolution review evidence, FailureAnalysis mutation,
...
- Resolution review input:
  ...
  - ReviewHistory ids and discrepancy ReviewHistory id.
- Resolution review fields:
  - resolution review id or artifact id when a later slice defines one;
  - resolution review action when a later slice defines one;
  - resolution action such as acknowledge_discrepancy,
    reject_discrepancy_resolution, request_discrepancy_clarification, or
  ...
  - ReviewHistory links;
  - failure code when resolution review input is invalid.
- Safety requirements:
  - resolution review records are evidence about human review of discrepancy
    records, not mutation of review summary exports, review decisions, audit
    summaries, prompt context consumption evidence, prompt context evidence, or
    underlying knowledge evidence;
  - resolution actions must not create prompt eligibility, approve
  ...
  - accepted/rejected/acknowledged discrepancy ids, affected citation ids,
    questioned/rejected citation groups, unresolved follow-up flags,
    unsupported claims, skipped evidence, source hashes, and mismatch reasons
    must remain visible instead of being filtered, deleted, or rewritten;
  - invalid, stale, mismatched, unsafe, revoked, cross-project, unsupported, or
    incomplete input must fail without appending a successful resolution review.
- Outputs:
  - prompt context discrepancy resolution review artifact id when a later
    scoped workflow persists one;
  - resolution action;
  - resulting resolution status;
  - accepted discrepancy ids;
  - rejected discrepancy ids;
...
- Runtime boundary:
  - this slice defines resolution review artifact and contract semantics only;
...
|---|---|---|---|---|
| Add TestKnowledgeCard prompt context discrepancy resolution review task plan | done | `test -f docs/implementation/slices/slice-47-test-knowledge-card-prompt-context-discrepancy-resolution-review-contract.md && rg -n "TestKnowledgeCard Prompt Context Discrepancy Resolution Review|review discrepancy|resolution review|resolution action|discrepancy type|affected citation ids|evidence gap summary|mismatch reason|severity|resolution status|ReviewHistory|Product Value Answer|Non-goals|Task Table" docs/implementation/slices/slice-47-test-knowledge-card-prompt-context-discrepancy-resolution-review-contract.md NEXT_AI_TASK.md && git diff --check` | `0a5c62c` | planning-only scope |
| Define TestKnowledgeCard prompt context discrepancy resolution review contracts | done | `rg -n "TestKnowledgeCard Prompt Context Discrepancy Resolution Review|prompt_context_discrepancy_resolution_review|review_prompt_context_discrepancy_resolution|resolution action|accepted discrepancy ids|rejected discrepancy ids|acknowledged discrepancy ids|affected citation ids|evidence gap summary|mismatch reason|resolution status|ReviewHistory|source hash|context manifest" docs/contracts/01-data-model-contract.md docs/contracts/02-api-contract.md docs/contracts/03-state-machines.md docs/contracts/04-artifact-contract.md docs/contracts/05-prompt-skill-contract.md docs/implementation/slices/slice-47-test-knowledge-card-prompt-context-discrepancy-resolution-review-contract.md && git diff --check` | pending | contract-only |
| Add TestKnowledgeCard prompt context discrepancy resolution review golden smoke | planned | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_prompt_context_discrepancy_resolution_review_contract_golden.py -q && git diff --check` | pending | no frontend/report/export runtime |
| Slice 47 completion gate | planned | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_prompt_context_discrepancy_resolution_review_contract_golden.py backend/app/tests/golden/test_test_knowledge_card_prompt_context_review_discrepancy_tracking_contract_golden.py -q && git diff --check` | pending | docs and handoff |

## Task 1: Add TestKnowledgeCard Prompt Context Discrepancy Resolution Review Task Plan

Goal: Create a narrow plan for prompt context discrepancy resolution review
...
- Slice 47 plan exists with product value, non-goals, task table, expected
...
```text
docs(v2): add test knowledge card prompt context discrepancy resolution review plan
```

## Task 2: Define TestKnowledgeCard Prompt Context Discrepancy Resolution Review Contracts

Goal: Update data, API, state-machine, artifact, and prompt/skill contracts
with the future resolution review boundary for prompt context discrepancy
records.
...
## Task 3: Add TestKnowledgeCard Prompt Context Discrepancy Resolution Review Golden Smoke

Goal: Add a contract-level fixture and golden smoke proving prompt context
discrepancy resolution review cannot mutate discrepancy tracking evidence,
review summary exports, review decisions, audit summaries, create prompt
eligibility, rewrite `used_knowledge`, or imply frontend/report/export runtime
behavior.
...
```bash
backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_prompt_context_discrepancy_resolution_review_contract_golden.py -q
git diff --check
```
...
Goal: Validate Slice 47 and hand off the next narrow V2 task.
...
```bash
backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_prompt_context_discrepancy_resolution_review_contract_golden.py backend/app/tests/golden/test_test_knowledge_card_prompt_context_review_discrepancy_tracking_contract_golden.py -q
git diff --check
```
