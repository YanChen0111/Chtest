...
| Add TestKnowledgeCard handoff golden smoke | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_handoff_contract_golden.py -q && git diff --check` | `87f2b20` | no CRUD/runtime |
| Slice 36 completion gate | done | `backend/.venv/bin/python -m pytest backend/app/tests/golden/test_test_knowledge_card_handoff_contract_golden.py backend/app/tests/golden/test_knowledge_feedback_review_gate_contract_golden.py -q && git diff --check` | pending | docs and handoff |
