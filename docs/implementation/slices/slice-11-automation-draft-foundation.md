...
|---|---|---|---|---|
| Add AutomationDraft task plan | done | `test -f docs/implementation/slices/slice-11-automation-draft-foundation.md && rg -n "AutomationDraft Foundation|Task Table|Verification Command|Non-goals" docs/implementation/slices/slice-11-automation-draft-foundation.md` | pending commit | scoped Slice 11 plan |
| Add AutomationDraft model and schema alignment | planned | `backend/.venv/bin/python -m pytest backend/app/tests/api/test_automation_draft.py -q` | - | model/schema/read contracts only |
| Add AutomationDraft generation API | planned | `backend/.venv/bin/python -m pytest backend/app/tests/api/test_automation_draft.py -q` | - | mock draft from reviewed TestCase |
