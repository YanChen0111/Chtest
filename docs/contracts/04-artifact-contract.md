...
Generated case human review decision artifact rules:

- `generated_case_human_review_decision.json` may be stored as an Artifact
  with `artifact_type=generated_case_human_review_decision`,
  `owner_entity_type=GeneratedCaseCandidate`, and
  `owner_entity_id=candidate_id`, and
  `manifest_kind=generated_case_human_review_decision` in a later scoped
  implementation. A future workflow may use `owner_entity_type=AITask` or
  `owner_entity_type=Project`, but it must preserve the GeneratedCaseCandidate
  id and `generated_case_human_review_evidence_package_artifact_id` in
  metadata.
- The artifact must include
...
- Generated case human review decision artifacts must be bounded JSON. They
  must not contain raw LLM/provider payloads, unbounded source text,
  credentials, API keys, tokens, OAuth state, remote fetch payloads, vector
  store payloads, embedding vectors, reranker traces, graph runtime payloads,
  executable prompt assembly payloads, runtime `prompt_input.json`,
  frontend-rendered markup, report-rendered payloads, export-rendered
  payloads, downloadable provider payloads, or generated replacement evidence.
- Generated case human review decision artifacts must not approve or reject
  ...
  mutate Artifact rows outside declared decision evidence, upload artifacts,
  run prompt assembly, execute AITasks, call providers, call provider SDKs,
  fetch remote URLs, change retrieval ranking, create vector indexes, create
  embeddings, rerank, run background indexing, run graph jobs, invoke MCP
...
TestKnowledgeCard Prompt Context Discrepancy Resolution Review artifact rules:

- `test_knowledge_card_prompt_context_discrepancy_resolution_review.json` may
  be stored as an Artifact with
  `artifact_type=test_knowledge_card_prompt_context_discrepancy_resolution_review`,
  `owner_entity_type=AITask` or `owner_entity_type=Project`, and
  `manifest_kind=test_knowledge_card_prompt_context_discrepancy_resolution_review`
  in a later scoped implementation.
- The prompt context discrepancy resolution review artifact must include
  `review_prompt_context_discrepancy_resolution`, prompt request id or AITask
  id when available, prompt context review discrepancy artifact id, prompt
  context audit review summary export artifact id, prompt context audit review
  ...
  resolution status, resolution action, accepted discrepancy ids, rejected
  discrepancy ids, acknowledged discrepancy ids, clarification requested
  fields, resulting resolution status, unresolved follow-up flags, unsupported
  claim references, source hashes, PromptVersion id, SkillVersion id,
  ReviewHistory links, failure code, and visible reason when applicable.
- Resolution review artifacts are evidence about human discrepancy review. They
  must not invent citations, rewrite `used_knowledge`, promote unsupported
  claims, create prompt eligibility, approve TestKnowledgeCard content, approve
  generated cases, turn skipped evidence into cited evidence, auto-resolve
  discrepancies, render reports, or expose download endpoints.
- Prompt context discrepancy resolution review artifacts must not contain raw
  large source text, credentials, tokens, unsafe provider payloads, vector
  store payloads, embedding vectors, reranker traces, graph runtime payloads,
  executable prompt assembly payloads, provider request/response payloads,
  frontend-rendered markup, report-rendered payloads, downloadable provider
  payloads, or generated replacement evidence.
- Prompt context discrepancy resolution review artifacts must not mutate
  Artifact rows outside declared resolution review output, mutate source
...
TestKnowledgeCard Prompt Context Discrepancy Resolution Summary Export artifact rules:

- `test_knowledge_card_prompt_context_discrepancy_resolution_summary_export.json`
  may be stored as an Artifact with
  `artifact_type=test_knowledge_card_prompt_context_discrepancy_resolution_summary_export`,
  `owner_entity_type=AITask` or `owner_entity_type=Project`, and
  `manifest_kind=test_knowledge_card_prompt_context_discrepancy_resolution_summary_export`
  in a later scoped implementation.
- The prompt context discrepancy resolution summary export artifact must
  include `export_prompt_context_discrepancy_resolution_summary`, prompt
  request id or AITask id when available, prompt context discrepancy
  resolution review artifact id, prompt context review discrepancy artifact
  id, prompt context audit review summary export artifact id, prompt context
  audit review decision artifact id, prompt context audit summary artifact id,
  ...
  gap summary, mismatch reason, unsupported claim references, source hashes,
  PromptVersion id, SkillVersion id, ReviewHistory links, failure code, and
  visible reason when applicable.
- Resolution summary export artifacts are evidence packages about discrepancy
  resolution reviews. They must not invent citations, rewrite
  `used_knowledge`, promote unsupported claims, create prompt eligibility,
  approve TestKnowledgeCard content, approve generated cases, turn skipped
  evidence into cited evidence, auto-resolve discrepancies, render reports, or
  expose download endpoints.
- Prompt context discrepancy resolution summary export artifacts must not
  ...
  provider payloads, or generated replacement evidence.
- Prompt context discrepancy resolution summary export artifacts must not
  mutate Artifact rows outside declared summary export output, mutate source
  artifacts, mutate prompt context discrepancy resolution review artifacts,
  mutate prompt context review discrepancy artifacts, mutate review summary
  export artifacts, mutate prompt context audit review decision artifacts,
  mutate prompt context audit summary artifacts, mutate prompt context
  consumption artifacts, mutate prompt context evidence artifacts, mutate
  retrieval boundary artifacts, mutate prompt eligibility artifacts, rewrite
  creation artifacts, mutate historical ReviewHistory, FailureAnalysis, Report,
...
KnowledgeAdapter Provider Evaluation Plan artifact rules:

- `knowledge_adapter_provider_evaluation_plan.json` may be stored as an
  Artifact with `artifact_type=knowledge_adapter_provider_evaluation_plan`,
  `owner_entity_type=AITask` or `owner_entity_type=Project`, and
  `manifest_kind=knowledge_adapter_provider_evaluation_plan` in a later scoped
  implementation.
- The provider evaluation plan artifact must include
  ...
  links, provider suitability status, blocker reasons, failure code, and
  visible reason when applicable.
- Provider suitability status values are `not_evaluated`, `suitable`,
  `suitable_with_constraints`, `blocked`, `needs_revision`, and
  `unsupported`. They are evaluation labels only and must not enable a
  provider, call a provider, run runtime retrieval, create embeddings, or set
  `used_knowledge=true`.
- KnowledgeAdapter provider evaluation plan artifacts must not contain raw
  provider payloads, provider schemas as business truth, credentials, API keys,
  tokens, OAuth state, remote fetch payloads, vector store payloads, embedding
  vectors, reranker traces, graph runtime payloads, executable prompt assembly
  payloads, runtime `prompt_input.json`, frontend-rendered markup,
...
KnowledgeAdapter Provider Evaluation Review Summary Export artifact rules:

- `knowledge_adapter_provider_evaluation_review_summary_export.json` may be
  stored as an Artifact with
  ...
  in a later scoped implementation.
- The provider evaluation review summary export artifact must include
  `export_knowledge_adapter_provider_evaluation_review_summary`, provider
  evaluation review decision artifact id,
  ...
  safety questions, provider suitability summary, license/reference summary,
  ...
  summary, source manifest ids, source hashes, ReviewHistory summary,
  ReviewHistory links, failure code, and visible reason when applicable.
- Exported decision groups may include `accepted_for_planning`,
  `accepted_with_constraints`, `blocked`, `needs_revision`, and
  `unsupported`. Review summary status values may include `not_exported`,
  `exported_for_planning`, and `failed_validation`. They are audit labels only
  and must not enable a provider, call a provider, run runtime retrieval,
  create embeddings, generate reports, expose export/download endpoints, or
  set `used_knowledge=true`.
- KnowledgeAdapter provider evaluation review summary export artifacts must
  not contain raw provider payloads, credentials, API keys, tokens, OAuth
  state, remote fetch payloads, vector store payloads, embedding vectors,
  reranker traces, graph runtime payloads, executable prompt assembly
  payloads, runtime `prompt_input.json`, frontend-rendered markup,
  report-rendered payloads, export-rendered payloads, downloadable provider
  payloads, or generated replacement evidence.
- KnowledgeAdapter provider evaluation review summary export artifacts must
  not mutate Artifact rows outside declared summary export output, mutate the
  reviewed provider evaluation review decision artifact, mutate the reviewed
  provider evaluation plan artifact, mutate KnowledgeAdapterConfig runtime
  state, mutate KnowledgeEvidence rows, mutate TestKnowledgeCard rows, approve
  or reject GeneratedCaseCandidate rows, promote TestCase rows, create
  ToolInvocation rows, run prompt assembly, execute AITasks, call providers,
  call provider SDKs, fetch remote URLs, change retrieval ranking, create
  vector indexes, create embeddings, rerank, run background indexing, run graph
  jobs, invoke MCP runtime, render frontend pages, generate reports, expose
