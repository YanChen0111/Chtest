...
Slice 54 generated case human review evidence package rules:

- Generated Case Human Review Evidence Package is a contract-only evidence
  bundle for human review. It starts from same-project GeneratedCaseCandidate
  rows and review evidence; it does not approve or reject candidates, promote
  TestCase rows, create AutomationDraft rows, or change candidate status.
- The contract-only package action is
  `build_generated_case_human_review_evidence_package`. Future payloads may
  store
  `generated_case_human_review_evidence_package_action=build_generated_case_human_review_evidence_package`.
- Inputs must preserve GeneratedCaseCandidate ids, candidate status, candidate
  summary, title, priority, test type, precondition, steps, expected results,
  input data, tags, requirement refs, risk refs, AI reason, generation reason,
  covered risk ids, duplicate-of case id, `source_knowledge_evidence_ids`,
  ...
  candidate ids, automation readiness blockers, reviewer comments, prompt
  context evidence artifact ids, prompt context consumption artifact ids,
  prompt context audit summary artifact ids, prompt context audit review
  decision artifact ids, prompt context audit review summary export artifact
  ids, prompt context discrepancy resolution audit handoff artifact ids,
  source hashes, source manifest ids, and ReviewHistory ids or ReviewHistory
  links when available.
- Outputs may include generated case human review evidence package id or
  artifact id, `generated_case_human_review_evidence_package`, candidate
  summary, evidence chain completeness, missing evidence summary, conflicting
  evidence summary, review blocker summary, dedup/readiness summary, human
  review checklist, included artifact ids, excluded artifact reasons,
...
- `quality_score`, `review_findings_json`, `coverage_gap_notes`,
  ...
  checklist are review aids only. They must not create TestCase rows, approve
  candidates, reject candidates, request optimization, create automation
  drafts, execute automation, generate reports, run retrieval, create vector
  indexes, create embeddings, rerank, call providers, or mutate source
  evidence.
...
- KnowledgeAdapter Provider Evaluation Plan starts from inert provider
  candidate metadata. It evaluates future Haystack, LlamaIndex, GraphRAG, or
  other KnowledgeAdapter providers as policy/evidence records before any
  provider integration, provider SDK, credential, external call, vector
  database, embedding, reranking, background indexing, runtime retrieval,
  provider-backed prompt context behavior, frontend page, or package upgrade
  exists.
- The contract-only provider evaluation action is
  `evaluate_knowledge_adapter_provider_plan`. It may record
  `evaluate_provider_candidate`, `record_provider_evaluation`,
  `block_provider_candidate`, or `request_provider_evaluation_revision` as
  evaluation-only labels, but those labels must not enable providers, change
  KnowledgeAdapterConfig runtime behavior, call providers, or mark
  `used_knowledge=true`.
- Provider evaluation input must preserve candidate provider name, provider
  family, adapter type, provider version, adapter version, license name,
  license URL, license compatibility notes, reference intake URLs or
  documentation snapshot artifact ids, supported retrieval modes, supported
  source types, expected KnowledgeEvidence normalization fields,
  provider_state, disabled by default policy, network and credential policy,
  fallback behavior expectations, metrics to collect, safety and redaction
  requirements, source hash requirements, and ReviewHistory ids when human
  review exists.
- Provider evaluation outputs may include provider evaluation plan id or
  artifact id, evaluation action, provider suitability status, KnowledgeEvidence
  normalization notes, citation traceability requirements, redaction and safety
  requirements, metric set, blocker reasons, fallback behavior,
  provider_state recommendation, disabled by default decision, license review
  result, reference intake summary, source manifest ids, source hashes,
  ReviewHistory links, failure code, and visible reason.
- Provider suitability status values are `not_evaluated`, `suitable`,
  `suitable_with_constraints`, `blocked`, `needs_revision`, and
  `unsupported`. They are evaluation labels only and must not create provider
  enablement, runtime connectivity, retrieval evidence, prompt eligibility, or
  `used_knowledge=true`.
- Fallback and evidence labels may include `fallback_required`,
  `local_no_knowledge_fallback`, `normalization_required`,
  `citation_traceability_required`, and `license_review_required`. They must
  remain visible and must not be converted into successful provider readiness.
...
- Provider evaluation records must not copy raw provider payloads, provider
  schemas, credentials, secrets, API keys, OAuth material, remote fetch
  payloads, vector store payloads, embedding vectors, reranker traces, graph
  runtime payloads, executable prompt assembly payloads, frontend-rendered
  markup, report-rendered payloads, or generated replacement evidence.
- KnowledgeAdapter provider evaluation must not create TestKnowledgeCard rows,
  mutate KnowledgeEvidence rows, mutate Artifact rows outside declared
  provider evaluation evidence, approve GeneratedCaseCandidate rows, promote
  TestCase rows, execute ToolInvocation rows, generate Reports, update CI/CD
  state, call provider SDKs, create vector indexes, create embeddings, rerank,
...
TestKnowledgeCard Prompt Context Review Discrepancy Tracking rules:

- TestKnowledgeCard Prompt Context Review Discrepancy Tracking starts from
  existing prompt context review summary export, audit review decision, audit
  summary, and prompt context consumption evidence. It is a contract for future
  reviewable discrepancy records and is not a frontend page, report generation
  behavior change, export/download endpoint, prompt assembly implementation,
  prompt runtime execution, provider behavior, retrieval ranking, card
  creation, broad CRUD, vector indexing, embedding, reranking, graph runtime,
  or MCP runtime.
- The contract-only discrepancy action is
  `track_prompt_context_review_discrepancy`. It may record discrepancy type,
  affected citation ids, evidence gap summary, mismatch reason, reviewer note,
  severity, resolution status, unresolved follow-up flags, unsupported claim
  references, and source hash/context manifest references, but it must not
  write a runtime `prompt_input.json`, expose a download endpoint, render a
  report, call providers, run AITasks, mutate TestKnowledgeCard rows, mutate
  review summary export artifacts, mutate audit review decision artifacts,
  mutate audit summary artifacts, mutate prompt context consumption artifacts,
  mutate prompt context evidence artifacts, mutate source artifacts, rewrite
...
- Discrepancy input must preserve prompt request id or AITask id when
  ...
  references, unresolved follow-up flags, source hashes or source quote/hash
  pointers, PromptVersion id/name/version, SkillVersion id/name/version,
  ReviewHistory ids, and review decision ReviewHistory id.
- Discrepancy outputs may include discrepancy id or artifact id, discrepancy
  action, discrepancy type, affected citation ids, evidence gap summary,
  mismatch reason, reviewer note, severity, resolution status, unresolved
  follow-up flags, unsupported claim references, source manifest ids, source
  hashes, context manifest references, PromptVersion/SkillVersion trace,
  ReviewHistory links, and failure code when applicable.
- Allowed discrepancy types include `citation_mismatch`, `missing_evidence`,
...
- Allowed resolution status values include `open`, `needs_clarification`,
  `acknowledged`, `rejected`, and `resolved_by_later_review`. These statuses
  are audit labels only. They must not create prompt eligibility, approve
  TestKnowledgeCard content, approve generated cases, alter `used_knowledge`,
  mark skipped evidence as cited, or mutate prior review evidence.
- Missing, stale, unsafe, cross-project, revoked, unsupported, unbounded,
  citation-mismatched, context-mismatched, prompt-version-mismatched,
  skill-version-mismatched, redaction-failed, evidence-mismatched,
  audit-summary-mismatched, review-decision-mismatched, or summary-export-
  mismatched input must produce a failure code and must not append a successful
  discrepancy record.
- Prompt context review discrepancy records must not copy raw large source
  text, unsafe provider payloads, vector store payloads, embedding vectors,
  reranker traces, graph runtime payloads, secrets, credentials, tokens, OAuth
  material, executable prompt assembly payloads, frontend-rendered markup,
...
TestKnowledgeCard Prompt Context Discrepancy Resolution Summary Export rules:

- TestKnowledgeCard Prompt Context Discrepancy Resolution Summary Export starts
  from existing discrepancy resolution review evidence and referenced
  discrepancy, review summary export, audit review decision, audit summary,
  prompt context consumption, and prompt context evidence. It is a contract for
  future resolution summary export evidence and is not a frontend page, report
  generation behavior change, export/download endpoint, prompt assembly
  implementation, prompt runtime execution, provider behavior, retrieval
  ranking, card creation, broad CRUD, vector indexing, embedding, reranking,
  graph runtime, or MCP runtime.
- The contract-only resolution summary export action is
  `export_prompt_context_discrepancy_resolution_summary`. It may package
  ...
  resolution status group, source hashes, context manifest references,
  PromptVersion/SkillVersion trace, and ReviewHistory links, but it must not
  write a runtime `prompt_input.json`, expose a download endpoint, render a
  report, call providers, run AITasks, mutate TestKnowledgeCard rows, mutate
  ...
  prompt context consumption artifacts, mutate prompt context evidence
  artifacts, mutate source artifacts, rewrite `used_knowledge`, invent
  citations, auto-resolve discrepancies, or mutate historical evidence.
- Resolution summary export input must preserve prompt request id or AITask id
  when available, prompt context discrepancy resolution review artifact id,
  ...
  status, resolution action, resulting resolution status, accepted discrepancy
  ...
  reason, reviewer note, follow-up flags, unsupported claim references, source
  hashes or source quote/hash pointers, PromptVersion id/name/version,
  SkillVersion id/name/version, ReviewHistory ids, discrepancy ReviewHistory
  id, and resolution review ReviewHistory id.
- Resolution summary export outputs may include resolution summary export id
  or artifact id, resolution summary export action, resolution outcome
  ...
  unresolved follow-up flag group, reviewer comment summary, resulting
  resolution status group, source manifest ids, source hashes, context manifest
  references, PromptVersion/SkillVersion trace, ReviewHistory links, failure
  code, and visible reason when applicable.
- Exported resolution outcome groups are evidence package labels only. They
  must not create prompt eligibility, approve TestKnowledgeCard content,
  approve generated cases, alter `used_knowledge`, mark skipped evidence as
  cited, invent citations, mutate prior review evidence, or automatically
  resolve other discrepancies.
- Accepted discrepancy group, rejected discrepancy group, acknowledged
  discrepancy group, clarification requested field group, resulting resolution
  status group, affected citation ids, unresolved follow-up flags, unsupported
  claim references, evidence gap summary, and mismatch reason must remain
  visible.
- Missing, stale, unsafe, cross-project, revoked, unsupported, unbounded,
  citation-mismatched, context-mismatched, prompt-version-mismatched,
  skill-version-mismatched, redaction-failed, discrepancy-mismatched,
  ...
  summary export.
- Prompt context discrepancy resolution summary export records must not copy
  raw large source text, unsafe provider payloads, vector store payloads,
  embedding vectors, reranker traces, graph runtime payloads, secrets,
  credentials, tokens, OAuth material, executable prompt assembly payloads,
  frontend-rendered markup, report-rendered payloads, downloadable provider
  payloads, or generated replacement evidence.

TestKnowledgeCard Prompt Context Discrepancy Resolution Audit Handoff rules:

- TestKnowledgeCard Prompt Context Discrepancy Resolution Audit Handoff starts
  from existing discrepancy resolution summary export evidence and referenced
  resolution review, review discrepancy, audit review summary export, audit
  review decision, audit summary, prompt context consumption, prompt context
  evidence, source hashes, context manifests, PromptVersion, SkillVersion, and
  ReviewHistory evidence. It is a contract for future final audit handoff
  evidence and is not a frontend page, report generation behavior change,
  export/download endpoint, external archive integration, prompt assembly
  implementation, prompt runtime execution, provider behavior, retrieval
  ranking, card creation, broad CRUD, vector indexing, embedding, reranking,
  graph runtime, or MCP runtime.
- The contract-only audit handoff action is
  `build_prompt_context_discrepancy_resolution_audit_handoff`. It may package
  a handoff summary, evidence chain status, included artifact ids, excluded
  artifact reasons, unresolved follow-up flags, unresolved evidence gaps,
  unsupported claim references, source manifest ids, source hashes, context
  manifest references, PromptVersion/SkillVersion trace, and ReviewHistory
  links, but it must not write a runtime `prompt_input.json`, expose a
  download endpoint, render a report, call providers, run AITasks, mutate
  TestKnowledgeCard rows, mutate prompt context discrepancy resolution summary
  export artifacts, mutate prompt context discrepancy resolution review
  artifacts, mutate prompt context review discrepancy artifacts, mutate review
  summary export artifacts, mutate audit review decision artifacts, mutate
  audit summary artifacts, mutate prompt context consumption artifacts, mutate
  prompt context evidence artifacts, mutate source artifacts, rewrite
  `used_knowledge`, invent citations, auto-resolve discrepancies, or mutate
  historical evidence.
- Audit handoff input must preserve prompt request id or AITask id when
  available, prompt context discrepancy resolution summary export artifact id,
  prompt context discrepancy resolution review artifact id, prompt context
  review discrepancy artifact id, prompt context audit review summary export
  ...
  follow-up flags, unsupported claim references, source hashes or source
  quote/hash pointers, PromptVersion id/name/version, SkillVersion
  id/name/version, ReviewHistory ids, discrepancy ReviewHistory id,
  resolution review ReviewHistory id, and summary export ReviewHistory id.
- Audit handoff outputs may include audit handoff id or artifact id, audit
  handoff action, handoff summary, evidence chain status, included artifact
  ids, excluded artifact reasons, unresolved follow-up flags, unresolved
  evidence gaps, unsupported claim references, source manifest ids, source
  hashes, context manifest references, PromptVersion/SkillVersion trace,
...
- Evidence chain status values are `complete`, `incomplete`, `blocked`, and
  `failed_validation`. They are audit labels only and must not create prompt
  eligibility, approve TestKnowledgeCard content, approve generated cases,
  alter `used_knowledge`, mark skipped evidence as cited, invent citations,
  mutate prior review evidence, or automatically resolve discrepancies.
- Included artifact ids, excluded artifact reasons, unresolved follow-up
  ...
  hashes, evidence gap summary, mismatch reason, PromptVersion/SkillVersion
  trace, context manifest references, and ReviewHistory links must remain
  visible.
- Missing, stale, unsafe, cross-project, revoked, unsupported, unbounded,
  citation-mismatched, context-mismatched, prompt-version-mismatched,
  skill-version-mismatched, redaction-failed, discrepancy-mismatched,
  ...
  audit-handoff-mismatched, or evidence-mismatched input must produce a
  failure code and visible reason and must not append a successful audit
  handoff.
- Prompt context discrepancy resolution audit handoff records must not copy raw
  large source text, unsafe provider payloads, vector store payloads, embedding
  vectors, reranker traces, graph runtime payloads, secrets, credentials,
  tokens, OAuth material, executable prompt assembly payloads,
  frontend-rendered markup, report-rendered payloads, external archive
...
Generated Case Human Review Evidence Package Artifact rule:

- Slice 54 generated case human review evidence packages may use
  `artifact_type=generated_case_human_review_evidence_package` in a later
  scoped implementation.
- `owner_entity_type=GeneratedCaseCandidate` with
  `owner_entity_id=candidate_id` is preferred. `owner_entity_type=AITask` or
  `owner_entity_type=Project` may be used by a later scoped package workflow,
  but metadata must preserve the GeneratedCaseCandidate id.
- `metadata_json` must include
  `created_by_component=GeneratedCaseHumanReviewEvidencePackage`,
  `generated_case_human_review_evidence_package_action=build_generated_case_human_review_evidence_package`,
  GeneratedCaseCandidate id, candidate status, candidate summary,
  ...
  ids, prompt context consumption artifact ids, prompt context audit summary
  artifact ids, prompt context audit review decision artifact ids, prompt
  context audit review summary export artifact ids, prompt context discrepancy
  resolution audit handoff artifact ids, evidence chain completeness, missing
  evidence summary, conflicting evidence summary, review blocker summary,
  dedup/readiness summary, human review checklist, included artifact ids,
  excluded artifact reasons, source manifest ids, source hashes,
  ReviewHistory links, failure code, and visible reason when applicable.
- This artifact is human review evidence only. It must not approve or reject
  ...
  upload artifacts, run prompts, execute AITasks, run retrieval, call
...
- Slice 50 provider evaluation planning may use
  `artifact_type=knowledge_adapter_provider_evaluation_plan` in a later scoped
  implementation.
- `owner_entity_type=AITask` or `owner_entity_type=Project` until a later
  scoped evaluation workflow owns a dedicated provider evaluation entity.
- `metadata_json` must include
  `created_by_component=KnowledgeAdapterProviderEvaluationPlan`,
  `knowledge_adapter_provider_evaluation_plan_action=evaluate_knowledge_adapter_provider_plan`,
  candidate provider name, provider family, adapter type, provider version,
  adapter version, license review result, reference intake summary,
  documentation snapshot artifact ids, expected KnowledgeEvidence
  normalization fields, provider_state recommendation, disabled by default
  decision, fallback behavior summary, metrics plan, blocker reasons, source
  manifest ids, source hashes, ReviewHistory links, failure code, and visible
  reason when applicable.
- This artifact is provider evaluation evidence only. It must not enable a
  provider, change runtime retrieval, call provider SDKs, fetch remote URLs,
...
KnowledgeAdapter provider evaluation review audit handoff Artifact rule:

- Slice 53 provider evaluation review audit handoffs may use
  `artifact_type=knowledge_adapter_provider_evaluation_review_audit_handoff`
  in a later scoped implementation.
- `owner_entity_type=AITask` or `owner_entity_type=Project` until a later
  scoped provider evaluation workflow owns a dedicated audit handoff entity.
- `metadata_json` must include
  ...
  `knowledge_adapter_provider_evaluation_plan_artifact_id`, candidate provider
  name, provider family, adapter type, provider version, adapter version,
  review decision, review status, review summary status, exported decision
  groups, accepted constraints, blocked reasons, unsupported reasons,
  requested revision fields, unresolved safety questions, unresolved follow-up
  ...
  default summary, fallback summary, metrics summary, included artifact ids,
  excluded artifact reasons, evidence chain status, provider review decision
  group summary, unresolved blocker summary, unresolved safety question
  summary, source manifest ids, source hashes, ReviewHistory links, failure
  code, and visible reason when applicable.
- This artifact is provider evaluation review audit handoff evidence only. It
  must not enable a provider, change runtime retrieval, call provider SDKs,
  fetch remote URLs, create vector indexes, create embeddings, rerank, run
  graph jobs, invoke MCP runtime, write runtime `prompt_input.json`, mark
  `used_knowledge=true`, generate reports, expose export/download endpoints,
  ...
  KnowledgeEvidence rows, mutate historical evidence, mutate TestKnowledgeCard
  rows, create prompt eligibility, approve generated cases, add RBAC, create
...
TestKnowledgeCard prompt context review discrepancy Artifact rule:

- Slice 46 prompt context review discrepancy tracking may use
  `artifact_type=test_knowledge_card_prompt_context_review_discrepancy` in a
  later scoped implementation.
- `owner_entity_type=AITask` or `owner_entity_type=Project` until a later
...
- `metadata_json` must include `created_by_component=TestKnowledgeCardPromptContextReviewDiscrepancyTracking`,
  ...
  PromptVersion id, SkillVersion id, ReviewHistory links, and failure code when
  applicable.
- This artifact is discrepancy evidence only. It must not render frontend
  pages, change report generation behavior, expose an export/download endpoint,
  assemble prompt text, call providers, run an AITask, mutate review summary
  export evidence, mutate audit review decision evidence, mutate audit summary
  evidence, mutate prompt context consumption evidence, mutate prompt context
  evidence, mutate source artifacts, rewrite `used_knowledge`, invent
...
TestKnowledgeCard prompt context discrepancy resolution summary export Artifact rule:

- Slice 48 prompt context discrepancy resolution summary export may use
  `artifact_type=test_knowledge_card_prompt_context_discrepancy_resolution_summary_export`
  in a later scoped implementation.
- `owner_entity_type=AITask` or `owner_entity_type=Project` until a later
  scoped resolution summary export workflow owns a dedicated summary export
  entity.
- `metadata_json` must include `created_by_component=TestKnowledgeCardPromptContextDiscrepancyResolutionSummaryExport`,
  ...
  rejected discrepancy ids, acknowledged discrepancy ids, clarification
  requested fields, resolution outcome summary, accepted discrepancy group,
  rejected discrepancy group, acknowledged discrepancy group, clarification
  requested field group, unresolved follow-up flag group, reviewer comment
  summary, resulting resolution status group, affected citation ids, evidence
  gap summary, mismatch reason, source hashes, PromptVersion id,
  SkillVersion id, ReviewHistory links, failure code, and visible reason when
  applicable.
- This artifact is resolution summary export evidence only. It must not render
  frontend pages, change report generation behavior, expose an export/download
  endpoint, assemble prompt text, call providers, run an AITask, mutate prompt
  context discrepancy resolution review evidence, mutate prompt context review
  discrepancy evidence, mutate review summary export evidence, mutate audit
  review decision evidence, mutate audit summary evidence, mutate prompt
  context consumption evidence, mutate prompt context evidence, mutate source
  artifacts, rewrite `used_knowledge`, invent citations, auto-resolve
  discrepancies, create prompt eligibility, approve TestKnowledgeCard content,
  approve generated cases, or mutate TestKnowledgeCard rows.

TestKnowledgeCard prompt context discrepancy resolution audit handoff Artifact rule:

- Slice 49 prompt context discrepancy resolution audit handoff may use
  `artifact_type=test_knowledge_card_prompt_context_discrepancy_resolution_audit_handoff`
  in a later scoped implementation.
- `owner_entity_type=AITask` or `owner_entity_type=Project` until a later
  scoped audit handoff workflow owns a dedicated handoff entity.
- `metadata_json` must include
  `created_by_component=TestKnowledgeCardPromptContextDiscrepancyResolutionAuditHandoff`,
  `prompt_context_discrepancy_resolution_audit_handoff_action=build_prompt_context_discrepancy_resolution_audit_handoff`,
  prompt context discrepancy resolution summary export artifact id, prompt
  ...
  included artifact ids, excluded artifact reasons, evidence chain status,
  handoff summary, source hashes, source manifest ids, context manifest
  references, PromptVersion id, SkillVersion id, ReviewHistory links, failure
  code, and visible reason when applicable.
- This artifact is audit handoff evidence only. It must not render frontend
  pages, change report generation behavior, expose an export/download
  endpoint, integrate with an external archive, assemble prompt text, call
  providers, run an AITask, mutate prompt context discrepancy resolution
  summary export evidence, mutate prompt context discrepancy resolution review
  evidence, mutate prompt context review discrepancy evidence, mutate review
  summary export evidence, mutate audit review decision evidence, mutate audit
  summary evidence, mutate prompt context consumption evidence, mutate prompt
  context evidence, mutate source artifacts, upload artifacts, rewrite
  `used_knowledge`, invent citations, auto-resolve discrepancies, create
  prompt eligibility, approve TestKnowledgeCard content, approve generated
  cases, or mutate TestKnowledgeCard rows.
