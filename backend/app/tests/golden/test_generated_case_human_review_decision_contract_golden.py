...
FIXTURE_PATH = Path("docs/fixtures/43-generated-case-human-review-decision-golden.md")
SLICE_PATH = Path(
...
def test_golden_generated_case_human_review_decision_names_surfaces() -> None:
...
def test_golden_generated_case_human_review_decision_has_required_terms() -> None:
    fixture = _read(FIXTURE_PATH)
    slice_plan = _read(SLICE_PATH)
    contracts = _contracts_text()
...
def test_golden_generated_case_human_review_decision_locks_api_payload_fields() -> None:
    fixture = _read(FIXTURE_PATH)
    ...
    _assert_normalized_contains(
        api_contract,
        "Generated Case Human Review Decision response shape",
    )
...
def test_golden_generated_case_human_review_decision_forbids_runtime_side_effects() -> None:
    fixture = _read(FIXTURE_PATH)
    contracts = _contracts_text()
    ...
    _assert_normalized_contains(api_contract, "must not append a successful decision")
    _assert_normalized_contains(api_contract, "human-review decision evidence only")
    _assert_normalized_contains(
    ...
    _assert_normalized_contains(
        state_contract, "generated_case_human_review_decision_failed_validation"
    )
    _assert_normalized_contains(
        artifact_contract, "artifact_type=generated_case_human_review_decision"
    )
    _assert_normalized_contains(
        artifact_contract, "manifest_kind=generated_case_human_review_decision"
    )
    _assert_normalized_contains(
        artifact_contract, "generated_case_human_review_decision.json"
    )
    ...
    _assert_normalized_contains(
        ...
        (
            ...
            "when the decision evidence is produced by a prompt or skill"
        ),
    )
...
def _alias(term: str) -> str:
    aliases = {
        ...
        "decision labels": "Allowed decision labels",
        "reviewer label/comment": "reviewer label, reviewer comment",
        "rejection/blocker reasons": "rejection reasons",
