...
def test_golden_test_knowledge_card_prompt_context_evidence_names_surfaces() -> None:
...
def test_golden_test_knowledge_card_prompt_context_evidence_has_required_terms() -> None:
    fixture = _read(FIXTURE_PATH)
    slice_plan = _read(SLICE_PATH)
    contracts = _contracts_text()
...
def test_golden_test_knowledge_card_prompt_context_evidence_forbids_side_effects() -> None:
    fixture = _read(FIXTURE_PATH)
    slice_plan = _read(SLICE_PATH)
    contracts = _contracts_text()
    ...
    _assert_normalized_contains(data_contract, "build_prompt_context_evidence")
    _assert_normalized_contains(api_contract, "must not write a runtime")
    _assert_normalized_contains(state_contract, "prompt_context_evidence_omitted")
    _assert_normalized_contains(
        artifact_contract, "artifact_type=test_knowledge_card_prompt_context_evidence"
    )
    _assert_normalized_contains(
        prompt_skill_contract, "must not set `used_knowledge=true`"
    )
    _assert_normalized_contains(contracts, "bounded snippet")
    _assert_normalized_contains(contracts, "source hash")
    _assert_normalized_contains(contracts, "omission reason")


def _alias(term: str) -> str:
    aliases = {
        "prompt_context_evidence": "prompt_context_evidence_action",
        "retrieval boundary artifact": "retrieval boundary artifact id",
        "context manifest": "context manifest artifact id",
        "failure behavior": "failure behavior",
        "PromptVersion name/version": "PromptVersion name/version",
        "SkillVersion name/version": "SkillVersion name/version",
        "test_knowledge_card_retrieval_boundary.json": (
            "test_knowledge_card_retrieval_boundary"
        ),
        "safe bounded snippet": "safe bounded snippet",
        "test_knowledge_card_prompt_context_evidence.json": (
            "test_knowledge_card_prompt_context_evidence.json"
        ),
        "artifact_type=test_knowledge_card_prompt_context_evidence": (
