...
def test_golden_test_knowledge_card_prompt_context_review_discrepancy_tracking_names_surfaces() -> None:
...
def test_golden_test_knowledge_card_prompt_context_review_discrepancy_tracking_has_required_terms() -> None:
    fixture = _read(FIXTURE_PATH)
    slice_plan = _read(SLICE_PATH)
    contracts = _contracts_text()
...
def test_golden_test_knowledge_card_prompt_context_review_discrepancy_tracking_forbids_side_effects() -> None:
    fixture = _read(FIXTURE_PATH)
    contracts = _contracts_text()
    ...
    _assert_normalized_contains(
        data_contract, "track_prompt_context_review_discrepancy"
    )
    _assert_normalized_contains(
        api_contract, "must not append a successful discrepancy record"
    )
    _assert_normalized_contains(
        state_contract, "prompt_context_review_discrepancy_failed"
    )
    _assert_normalized_contains(
        artifact_contract,
        "artifact_type=test_knowledge_card_prompt_context_review_discrepancy",
...
def _alias(term: str) -> str:
    aliases = {
        ...
        "review summary export artifact": "review summary export artifact id",
        "audit review decision artifact": (
        ...
        "discrepancy artifact id": "discrepancy id or artifact id",
        "prompt context review discrepancy artifact id": (
            "prompt_context_review_discrepancy_artifact_id"
        ),
        "test_knowledge_card_prompt_context_review_discrepancy.json": (
