...
def test_workflow_queue_routes_only_exact_same_project_case_review() -> None:
    with _session() as session:
        project, other_project = _projects(session)
        requirement, review = _requirement_review(session, project)
        _, other_review = _requirement_review(session, other_project)
        ...
        def queued(subject_ref: str, *, stage: ControlledStage = ControlledStage.CASE_REVIEW):
            return submit_for_review(
                session,
                project.id,
                _create_run(
...
def test_workflow_queue_routes_only_exact_same_project_automation_plan_review() -> None:
    with _session() as session:
        project, other_project = _projects(session)
        requirement, review = _requirement_review(session, project)
        _, other_review = _requirement_review(session, other_project)
        ...
        def queued(
            subject_ref: str,
            *,
            stage: ControlledStage = ControlledStage.AUTOMATION_PLAN_REVIEW,
        ):
            return submit_for_review(
                session,
                project.id,
...
def test_workflow_queue_routes_only_exact_same_project_automation_draft_review() -> None:
    with _session() as session:
        project, other_project = _projects(session)
        requirement, review = _requirement_review(session, project)
        _, other_review = _requirement_review(session, other_project)
        ...
        def queued(
            subject_ref: str,
            *,
            stage: ControlledStage = ControlledStage.AUTOMATION_DRAFT_REVIEW,
        ):
            return submit_for_review(
                session,
                project.id,
        ...
        expected_route = (
            f"/automation/drafts?requirement_id={requirement.id}"
            f"&requirement_review_id={review.id}&workflow_run_id={valid.id}"
            "&workflow_stage=automation_draft_review"
        )
        valid_id = valid.id
        cross_project_id = cross_project.id
        inactive_id = inactive.id
...
def test_workflow_queue_routes_only_exact_same_project_execution_result_review() -> None:
    with _session() as session:
        project, other_project = _projects(session)
        requirement, review = _requirement_review(session, project)
        _, other_review = _requirement_review(session, other_project)
        ...
        def queued(
            subject_ref: str,
            *,
            stage: ControlledStage = ControlledStage.EXECUTION_RESULT_REVIEW,
        ):
            return submit_for_review(
                session,
                project.id,
...
def test_workflow_queue_routes_only_exact_same_project_report_review() -> None:
    with _session() as session:
        project, other_project = _projects(session)
        requirement, review = _requirement_review(session, project)
        _, other_review = _requirement_review(session, other_project)
        ...
        def queued(
            subject_ref: str,
            *,
            stage: ControlledStage = ControlledStage.REPORT_REVIEW,
        ):
            return submit_for_review(
                session,
                project.id,
