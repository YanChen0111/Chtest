...
- 第一版主线是需求到用例、用例到自动化；CI/CD 管理中的本地 Git diff 到质量报告是支线能力。
- 第一版必须包含 AI 用例生成评审窗口和质量指标。
- 第一版必须包含 AutomationDraft，AI 自动化草稿必须审批后执行。
- 第一版必须包含 CI/CD 管理，支持本地 diff 后单测生成和回归测试查看，但不能压过主线；远程 PR/云 CI 集成后置。
...
- CI/CD 管理变更有 diff 分析、UnitTestPatch、回归计划和报告。
- 测试执行有原始日志和结构化结果。
